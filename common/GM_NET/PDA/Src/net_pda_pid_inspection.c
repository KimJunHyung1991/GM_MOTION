...
{
	prtc_data_ctl_inspection_data_play_t *pCdcid = (prtc_data_ctl_inspection_data_play_t *)make_data_buff;
	pCdcid->index = index;
	for(int i = 0; i < 7; i++){
		pCdcid->data[i] = data[i];
	}	
...
{
	prtc_data_rsp_inspection_data_play_t *pCdrid = (prtc_data_rsp_inspection_data_play_t *)make_data_buff;
	pCdrid->index = index;
	for(int i = 0; i < 7; i++){
		pCdrid->data[i] = data[i];
	}
